...
def build_sensor_config(num_samples, num_frames_per_sample, base_config_dict):
    ...
    x_orig_center = np.random.uniform(0.15, 0.85)
    obs_list_origin_list[0]['low'] = x_orig_center + np.random.uniform(-0.05, 0.05)
    obs_list_origin_list[0]['high'] = x_orig_center + np.random.uniform(-0.05, 0.05)

    y_orig_center = np.random.uniform(0.15, 0.85)
    obs_list_origin_list[1]['low'] = y_orig_center + np.random.uniform(-0.05, 0.05)
    obs_list_origin_list[1]['high'] = y_orig_center + np.random.uniform(-0.05, 0.05)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_file_path', type=str,
                        default="/home/jfletcher/research/satsim_data_gen/satsim_cso.json",
